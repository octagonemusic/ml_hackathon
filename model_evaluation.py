...
def evaluate_model_performance(models, df):
    ...
    y_cl_pred = (
        models["TDS→Chloride"]["scaler_y"]
        .inverse_transform(
            models["TDS→Chloride"]["model"]
            .predict(X_tds_cl_poly)
            .reshape(-1, 1)
        )
        .ravel()
    )
...
def generate_latex_table(results):
    ...
    for metric in metrics:
        row = (
            f"{metric} & "
            + " & ".join([f"{results[model][metric]:.4f}" for model in models])
            + " \\\\\n"
        )
        latex_table += row
...
def plot_residual_analysis(models, df):
    ...
    fig.suptitle(
        "Residual Analysis for Water Quality Prediction Models", fontsize=16
    )
...
def main():
    ...
    print(
        "Evaluation complete! Results saved in 'model_evaluation_results' directory"
    )
